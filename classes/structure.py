...
class Structure(object):
    ...
    def check_inclusion(self, p):
        ...
        '''
        raise Exception("aww, no implementation yet available for this method...")
    ...
    def ccs(self, use_lib=True, impact_path='', impact_options="-Octree -nRuns 32 -cMode sem -convergence 0.01", pdbname="", scale=False, proberad=1.0):
        ...
        try:
            ...
            # clean temp files if needed (if a filename is provided, don't
            # delete it!)
    ...
    def saxs(self, crysol_path='~/atsas-2.5.2-1/bin/', crysol_options="-lm 20 -ns 500", pdbname=""):
        '''
        ...
        try:
            subprocess.check_call('%s/crysol %s %s >& /dev/null' %(crysol_path, crysol_options, pdbname), shell=True)
        except Exception as e:
